...
export default function PlaceDetailsScreen() {
    ...
    useEffect(() => {
        const fetchPlaceDetails = async () => {
            if (!id) return;
            try {
                const docRef = doc(db, 'places', id as string);
                ...
                if (docSnap.exists()) {
                    ...
                    setPlace({
                        title: data.title || (paramTitle as string) || 'Place',
                        distance: data.distance || data.location || 'Distance info not available',
                        rating: data.rating || 4.5,
                        location: data.location || 'Location details not available',
                        time: data.time || '9 AM - 6 PM',
                        description: data.description || 'No description available.',
                        historicalStory: data.historicalStory || data.description || 'No historical info available.',
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                ...
                <View style={styles.headerContainer}>
                    {place.images && place.images.length > 0 ? (
                        <Image source={{ uri: place.images[0] }} style={styles.headerImage} resizeMode="cover" />
                    ) : (
